...
Modified by Erfan Azad on 17 Dec 2014
*/
...
void _mergesort(double lyst[], int size);
...
int main (int argc, char *argv[])
{
	...
	int T_LEVEL = THREAD_LEVEL;
	if(argc == 3){//user specified Thread number
		NUM = atoi(argv[1]);
		T_LEVEL = atoi(argv[2]);
	}
	...
	//number of threads and size of the problem
	...
	{
		printf("Oops, list did not get sorted by mergesort.\n");
	}
	...
	gettimeofday(&start, NULL);
	parallelMergesort(lyst, NUM, T_LEVEL);
	gettimeofday(&end, NULL);
